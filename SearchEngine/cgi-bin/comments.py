...
def cluster_points(L):
    ...
    for w in range(0, 200):
        rIdx = kmeans2(Y, 2, iter=5, minit='random')[1].tolist()
        newMod = gr.modularity(rIdx)
        if newMod > mod:
            mod = newMod
    #        print mod
            returnIdx = rIdx
...
con = mdb.connect(host='localhost', port=3307, user='sincereRead',
                  db='sincere', passwd='', unix_socket="/tmp/mariadb3307.sock")
cur = con.cursor(mdb.cursors.DictCursor)
#print 'select comment.id as CommentID, likedby.fb_id as LikeFromID, comment.fb_id as LikeToID from comment, likedby where likedby.page_id='+con.escape_string(pageID)+' AND likedby.post_id='+con.escape_string(postID)+' AND comment.id = likedby.comment_id AND comment.page_id=likedby.page_id AND comment.post_id=likedby.post_id;'
sql = ("SELECT comment.id AS CommentID, IFNULL(likedby.fb_id, 0) AS LikeFromID, "
      "comment.fb_id AS LikeToID FROM comment LEFT JOIN likedby ON "
      "(likedby.page_id = comment.page_id AND likedby.comment_id = comment.id "
      "AND comment.post_id=likedby.post_id) WHERE comment.page_id=" + con.escape_string(pageID) + " "
      "AND comment.post_id=" + con.escape_string(postID) + ";")
cur.execute(sql)
