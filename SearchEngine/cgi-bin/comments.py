...
status = {'modularity': 0, 'message': "Ok", 'id': str(pageID) + '_' + str(postID)}
...
for rawlikedata_line in rawlikedata:
  gr.add_edges((FromNodeIDtoVerticeID[rawlikedata_line['LikeFromID']],FromNodeIDtoVerticeID[rawlikedata_line['LikeToID']]))
#print summary(gr)
...
s =json.dumps({'status':status,'comments':result, 'influence': influence},  sort_keys=False,indent=2 * ' ')
